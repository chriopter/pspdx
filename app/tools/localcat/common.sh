# The parts tools/localcat/run and dev/start --mock both need: one work
...
#
#   APP="$HERE/app"; . "$APP/tools/localcat/common.sh"
: "${APP:?tools/localcat/common.sh needs APP set to app/}"
LOCALCAT="$APP/tools/localcat"
...
localcat_build_local() {
	localcat_certs
	# make runs inside the container with app/ as the working directory, so
	# the header has to be named relative to app/ -- and therefore has to be
	# under it, which is where the work directory lives unless it was moved.
	rel="${WORK#"$APP"/}"
	if [ "$rel" = "$WORK" ]; then
